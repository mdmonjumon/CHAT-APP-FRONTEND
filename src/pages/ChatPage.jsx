...
import { Send, Image, Smile, MoreVertical, Search, Phone, Video, Menu, X, LogOut, Settings } from 'lucide-react';
...
const ChatPage = () => {
  ...
  const handleLogout = () => {
    ...
    // Your logout logic here (e.g., clearing tokens)
  };
    ...
    <div className="h-screen bg-base-200 flex overflow-hidden relative">
      ...
      `}>
        ...
        <div className="p-4 border-b border-base-300 flex justify-between items-center">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-primary rounded-lg flex items-center justify-center text-primary-content font-bold">C</div>
            <h1 className="text-xl font-bold">Chatly</h1>
          </div>
          <button className="lg:hidden btn btn-ghost btn-sm btn-circle" onClick={() => setIsSidebarOpen(false)}>
            <X size={20} />
          </button>
        </div>
        ...
        <div className="overflow-y-auto flex-1">
          {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
            <div key={i} onClick={() => setIsSidebarOpen(false)} className="flex items-center gap-3 p-4 hover:bg-base-200 cursor-pointer transition-colors">
              <div className="avatar online">
                <div className="w-12 rounded-full"><img src={`https://i.pravatar.cc/150?u=${i + 20}`} /></div>
              </div>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm opacity-60 truncate">Message preview text...</p>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="flex items-center justify-between group">
            <div className="flex items-center gap-3">
              ...
              <div className="text-left overflow-hidden">
                <p className="font-bold text-sm truncate w-32">{currentUser.name}</p>
                <p className="text-xs opacity-50 truncate w-32">{currentUser.email}</p>
              </div>
            </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-base-100">
        <div className="p-4 border-b border-base-300 flex items-center gap-3">
            <button className="btn btn-ghost btn-circle lg:hidden" onClick={() => setIsSidebarOpen(true)}>
              <Menu size={24} />
            </button>
            <div className="avatar online"><div className="w-10 rounded-full"><img src="https://i.pravatar.cc/150?u=21" /></div></div>
            <h3 className="font-bold">{currentUser?.name}</h3>
        </div>
        
        <div className="flex-1 overflow-y-auto p-4 bg-base-200/30">
          {/* Messages go here */}
          <div className="text-center opacity-30 mt-10 italic">Select a contact to start chatting</div>
        </div>
